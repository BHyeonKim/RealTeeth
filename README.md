...
**Query key factory**  
query key를 문자열 배열 리터럴로 직접 쓰면 오타가 생기거나 여러 곳에서 다른 형태로 쓰일 수 있습니다. `whetherQueryKeys` 객체로 factory 함수를 모아두면 key 생성 로직이 한 곳에 집중되고, 인자 타입도 강제됩니다.

```typescript
whetherQueryKeys.all                          // ['whether']
whetherQueryKeys.vilageForecastAll()          // ['whether', 'vilageForecast']
whetherQueryKeys.vilageForecast(coord, date, time)
// ['whether', 'vilageForecast', nx, ny, date, time]
```

TanStack Query의 `invalidateQueries`는 **전달한 key가 prefix인 쿼리를 전부 무효화**합니다. 계층 구조 덕분에 `vilageForecastAll()`을 넘기면 좌표나 시각에 무관하게 단기예보 캐시 전체를 한 번에 무효화할 수 있고, 특정 위치만 무효화하고 싶을 때는 좌표까지 포함한 key를 쓰면 됩니다.
...
```mermaid
flowchart LR
    subgraph naive ["resize + useState (비교)"]
        A[resize] -->|"모든 픽셀마다"| B[setState]
        B --> C[리렌더]
    end

    subgraph impl ["useSyncExternalStore (실제 구현)"]
        D[change] -->|"440px 경계 넘을 때만"| E[getSnapshot]
        E -->|"'mobile'↔'desktop' 변경 시만"| F[리렌더]
    end
```
